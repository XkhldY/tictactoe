...
class BoardViewset(views.APIView):
  ...
  def post(self, request):
    ...
    try:
      ...
      currnet_board = json.loads(data['current_board'])
      open_indexes = [i for i, j in currnet_board.items() if j == None]
    ...
    if not open_indexes:
      ...
      return Response(currnet_board.values(), status=status.HTTP_200_OK)

    else:
      index = random.choice(open_indexes)
      currnet_board[index] = "o"
